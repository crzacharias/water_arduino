...
float statistics(float X[], float sX)
{
  ...
  uncert = sqrt(uncert/(nread -1));  // standard deviations
  uncert = sqrt(uncert*uncert + sX*sX);  // adding sensor precision
...
void configuraModulo()
{
  ...
  */
  ...
  Wire.write(decToBcd(sec));      //convertendo os segundos.
  Wire.write(decToBcd(mnt));       //convertendo os minutos.
  Wire.write(decToBcd(hour));         //convertendo as horas.
  Wire.write(decToBcd(week));  //dia da semana, onde o domingo começa com "0".
  Wire.write(decToBcd(dmonth));     //convertendo o dia do mês.
  Wire.write(decToBcd(month));          //convertendo o mês.
  Wire.write(decToBcd(year));          //convertendo o year.
...
void imprimeDadosModulo()
{
  ...
  */
  ...
  sec = bcdToDec(Wire.read() & 0x7f); //Alguns dados precisam de máscaras antes 
                                          //da conversão porque certos bits são 
                                          //bits de controle. 
  mnt = bcdToDec(Wire.read());         //convertendo os minutos. 
  hour = bcdToDec(Wire.read() & 0x3f);    //Alguns dados precisam de máscaras antes 
                                          //da conversão porque certos bits são 
                                          //bits de controle. Essa máscara define o
                                          //relógio para trabalhar no modo de 24h.
  week = bcdToDec(Wire.read());    //dia da semana, onde domingo começa com "0".
  dmonth = bcdToDec(Wire.read());       //convertendo o dia do mês.
  month = bcdToDec(Wire.read());            //convertendo o mês.
  year = bcdToDec(Wire.read());            //convertendo o ano.
  ...
  ajustaHora += ajustaZero(hour); 
  Serial.print(ajustaHora); hour1 = ajustaHora;
  Serial.print(":");
  //chmando a função ajustaZero para o caso de dados gravado no módulo com apenas um dígito.
  ajustaMinuto += ajustaZero(mnt);
  Serial.print(ajustaMinuto); mnt1 = ajustaMinuto;
  Serial.print(":");
  //chmando a função ajustaZero para o caso de dados gravado no módulo com apenas um dígito.
  ajustaSegundo += ajustaZero(sec); sec1 = ajustaSegundo;
  ...
  switch(week)
  {
    case 0:Serial.println("Domingo"); break; //se a variável week for zero
  ...
  Serial.print("Data: ");
  //chmando a função ajustaZero para o caso de dados gravado no módulo com apenas um dígito.
  ajustaDiaDoMes += ajustaZero(dmonth); dia1 = dmonth;
  Serial.print(ajustaDiaDoMes);
  Serial.print("/");
  //chmando a função ajustaZero para o caso de dados gravado no módulo com apenas um dígito.
  ajustaMes += ajustaZero(month); month1 = month;
